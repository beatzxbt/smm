...
class TestRateLimiter:
    ...
    def test_simple_acquire_and_exhaustion(self, clock):
        ...
        clock.set(100)
        rl = RateLimiter(rate_per_sec=1, capacity=3)
    ...
    def test_refill_over_time(self, clock):
        ...
        clock.set(0)
        rl = RateLimiter(rate_per_sec=2, capacity=5)
    ...
    def test_time_to_availability_when_drained(self, clock):
        ...
        clock.set(0)
        rl = RateLimiter(rate_per_sec=2, capacity=3)

        # Drain
        assert rl.try_acquire(3) is True

        # No time advanced: need 0.5s for 1 token, 1.0s for 2 tokens
        assert rl.time_to_availability(1) == pytest.approx(0.5)
        assert rl.time_to_availability(2) == pytest.approx(1.0)
...
class TestRateLimiterEdgeCases:
    ...
    def test_invalid_initialization_raises(self, rate: int, cap: int):
    ...
    def test_zero_and_negative_token_requests(self, clock):
        ...
        clock.set(10)
        rl = RateLimiter(rate_per_sec=1, capacity=1)

        # Drain
        assert rl.try_acquire(1) is True
        # Zero/negative requests are treated as immediate success / zero wait
        assert rl.try_acquire(0) is True
        assert rl.try_acquire(-1) is True
        assert rl.time_to_availability(0) == pytest.approx(0.0)
        assert rl.time_to_availability(-2) == pytest.approx(0.0)
