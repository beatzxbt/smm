"""Binance market data stream runner for 300 alphabetically selected instruments.

Usage:
- Run `uv run python scripts/binance_market_stream.py`.
- Press Ctrl+C to stop.

Components:
- BinanceExchange + BinanceMarketStreamManager initialization.
...
def build_logger() -> Logger:
    ...
    """
    log_path = "logs/binance_market_stream.txt"
...
async def resolve_instruments(
    ...
    Args:
        exchange (BinanceExchange): Exchange client used for symbol resolution.
...
class StreamRunner:
    ...
    def __init__(self) -> None:
        ...
        self.exchange: BinanceExchange | None = None
        self.manager: BinanceMarketStreamManager | None = None
...
async def run(runner: StreamRunner) -> None:
    """Run the Binance market data stream and log messages.

    Args:
        runner (StreamRunner): Runner instance holding shared state.
    """
    runner.logger = build_logger()
    runner.exchange = BinanceExchange(
    ...
    try:
        runner.manager = await BinanceMarketStreamManager.create(
